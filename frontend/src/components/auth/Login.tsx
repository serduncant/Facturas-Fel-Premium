...
export const Login: React.FC<LoginProps> = ({ 
  ...
  return (
    <div className={`max-w-md mx-auto ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} rounded-3xl border shadow-2xl p-8`}>
      ...
      {/* Botones de Acceso Instantáneo Demo */}
      <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800 space-y-2">
        ...
        >
          ...
          <span>Probar como Usuario Demo</span>
        </button>
        ...
        <button
          ...
          onClick={() => {
            guestLogin('admin');
            toast.success('¡Bienvenido en Modo Admin Enterprise!');
          }}
          className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-indigo-950/60 hover:bg-indigo-900/60 text-indigo-300 border border-indigo-500/30 font-bold text-[11px] rounded-xl transition-all"
        >
          <Shield className="w-3.5 h-3.5 text-purple-400" />
          <span>Probar como Admin Enterprise</span>
        </button>
      </div>
