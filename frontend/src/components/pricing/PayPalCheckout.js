...
export const PayPalCheckout = ({ planName, amount, onSuccess, onCancel, darkMode = false }) => {
    ...
    return (_jsx("div", { className: "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4", children: _jsxs("div", { className: `${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg max-w-md w-full p-6`, children: [_jsxs("div", { className: "flex justify-between items-center mb-6", children: [_jsxs("div", { children: [_jsx("h3", { className: `text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`, children: "Pago con PayPal" }), _jsxs("p", { className: `text-sm mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`, children: ["Plan ", planName, " - $", amount, " USD (\u2248 Q", (amount * 7.8).toFixed(2), ")"] })] }), _jsx("button", { onClick: onCancel, className: "p-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors", disabled: processing, children: _jsx(X, { size: 20 }) })] }), _jsx("div", { className: "mb-4", children: _jsx("div", { className: `p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-blue-50'} border ${darkMode ? 'border-gray-600' : 'border-blue-200'}`, children: _jsx("p", { className: `text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`, children: "\u2139\uFE0F Ser\u00E1s redirigido a PayPal para completar tu pago de forma segura. Tu plan se activar\u00E1 autom\u00E1ticamente." }) }) }), processing && (_jsx("div", { className: `mb-4 p-4 rounded-lg ${darkMode ? 'bg-blue-900/20' : 'bg-blue-50'} border ${darkMode ? 'border-blue-800' : 'border-blue-200'}`, children: _jsx("p", { className: `text-sm ${darkMode ? 'text-blue-300' : 'text-blue-700'}`, children: "\u23F3 Procesando pago y activando plan... Por favor espera." }) })), _jsx(PayPalScriptProvider, { options: paypalOptions, children: _jsx(PayPalButtons, { createOrder: createOrder, onApprove: onApprove, onError: onError, onCancel: () => {
                            ...
                            shape: 'rect',
                            label: 'paypal'
                        } }) }), _jsx("p", { className: "text-xs text-center text-gray-500 mt-4", children: "Transacci\u00F3n segura procesada por PayPal" })] }) }));
};
