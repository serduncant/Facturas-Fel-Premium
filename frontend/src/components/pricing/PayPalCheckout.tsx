...
export const PayPalCheckout: React.FC<PayPalCheckoutProps> = ({
  ...
  const onApprove = async (_data: any, actions: any) => {
    ...
    try {
      ...
      const tierMap: { [key: string]: 'free' | 'basic' | 'premium' | 'enterprise' } = {
        'Free': 'free',
        'Basic': 'basic',
        'Pro': 'premium',
        'Max': 'enterprise'
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg max-w-md w-full p-6 shadow-2xl`}>
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className={`text-sm mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              Plan {planName} - ${amount} USD (≈ Q{(amount * 7.8).toFixed(2)})
            </p>
          </div>
          <button
            onClick={onCancel}
        ...
        {processing && (
          <div className={`mb-4 p-4 rounded-lg ${darkMode ? 'bg-blue-900/20' : 'bg-blue-50'} border ${darkMode ? 'border-blue-800' : 'border-blue-200'}`}>
            <p className={`text-sm ${darkMode ? 'text-blue-300' : 'text-blue-700'}`}>
              ⏳ Procesando pago y activando plan... Por favor espera.
            </p>
          </div>
        )}
