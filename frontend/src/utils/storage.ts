...
export const saveInvoice = (invoice: Invoice): void => {
  try {
    ...
    const exists = history.some(
      inv => inv.certificador.autorizacion === invoice.certificador.autorizacion
    );
...
export const deleteInvoice = (autorizacion: string): void => {
  try {
    const history = getInvoices();
    const filtered = history.filter(
      inv => inv.certificador.autorizacion !== autorizacion
    );
